...
swath_alphadiv <- cbind(swath_groups, swath_richness, swath_shannon, swath_evenness, swath_abund)%>%
...
upc_alphadiv <- cbind(upc_groups, upc_richness, upc_shannon, upc_evenness, upc_abund)%>%
  mutate(MHW = str_to_sentence(MHW),
         MHW = factor(MHW, levels=c("Before","During","After")))
...
fish_alphadiv <- cbind(fish_groups, fish_richness, fish_shannon, fish_evenness, fish_abund)%>%
  mutate(MHW = str_to_sentence(MHW),
         MHW = factor(MHW, levels=c("Before","During","After")),
...
overall <- ggplot(swath_alphadiv, aes(x = year, y=swath_shannon))+
  geom_point(alpha = 0.2, size=0.5) +
  geom_smooth(method = "auto", se = TRUE, size = 0.5, alpha = 0.3, inherit.aes = TRUE) +
  theme_bw() + my_theme

overall


swath_plot <- ggplot(swath_alphadiv, aes(x = year, y=swath_shannon))+
              geom_point(alpha = 0.2, size=0.5) +
              geom_smooth(method = "auto", se = TRUE, size = 0.5, alpha = 0.3, inherit.aes = TRUE) +
              facet_wrap(~site, scales="free_x")+ theme_bw() + my_theme
