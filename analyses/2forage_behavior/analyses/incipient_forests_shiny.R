...
################################################################################
#calculate post-MHW baseline average (2017-2019)


# Select only the layers from 2017 to 2020
stacked_raster_2017_2019 <- stacked_raster[[grep("year_201[7-8]|year_2019", names(stacked_raster))]]

# Define a function to calculate the average for each quarter
average_quarter <- function(x) {
  mean(x, na.rm = TRUE)
}

# Create an empty stacked raster with four layers
baseline_rast <- stack()
baseline_rast <- setExtent(baseline_rast, extent(stacked_raster_2017_2019)) # Set the extent of the output raster to match the input raster
proj4string(baseline_rast) <- proj4string(stacked_raster_2017_2019) # Set the projection of the output raster to match the input raster

# Loop over each quarter and calculate the average for each quarter across the years
for (i in 1:4) {
  quarter_layers <- stacked_raster_2017_2019[[grep(paste0("_quarter_", i), names(stacked_raster_2017_2019))]]
  quarter_averages <- calc(quarter_layers, average_quarter)
  baseline_rast <- addLayer(baseline_rast, quarter_averages) # Add the quarter raster to the output stack
  names(baseline_rast)[i] <- paste0("Q", i) # Set the name of the layer
}

# Set the extent and projection of the output raster to match the input raster
extent(baseline_rast) <- extent(stacked_raster)
proj4string(baseline_rast) <- proj4string(stacked_raster)
