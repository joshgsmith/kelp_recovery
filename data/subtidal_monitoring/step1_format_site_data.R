...
site_data <- read.csv(file.path(basedir, "data/subtidal_monitoring/raw/1999_2022_data/PISCO_kelpforest_combined_data_bysite.csv")) %>%
  janitor::clean_names() %>%
  #filter central coast only
  filter(mlpa_regn == "CCSR")
...
################################################################################
#bust up the meta data in site_data into parts


fish_dat_build1 <- site_data %>%
  dplyr::select(1:9, starts_with("fish_")) %>%
  #drop string
  rename_with(~ gsub("^fish_", "", .), starts_with("fish_")) %>%
  #drop any columns that are all NA
  dplyr::select(-which(sapply(., function(x) all(is.na(x))))) %>%
  #drop species that were never observed 
  dplyr::select(-which(sapply(., function(x) all(x == 0)))) %>%
  #make long
  pivot_longer(cols = c(10:ncol(.)), names_to="classcode", values_to="counts")%>%
  #drop yoy
  filter(!grepl("yoy$",classcode))

#join with species names
fish_dat_build2 <- left_join(fish_dat_build1, kelp_taxon, by="classcode") %>%
                    dplyr::select(!(classcode))%>%
  mutate(species_definition = tolower(str_replace_all(species_definition, " ", "_"))) %>%
  #check for any NAs in Viewer then reshape back to wide
  pivot_wider(names_from = "species_definition", values_from = "counts") %>%
                    #Drop species
  dplyr::select(!(c(
             'atherinopsidae',
...
swath_dat_build1 <- site_data %>%
  dplyr::select(1:9, starts_with("swath_")) %>%
  #drop string
  rename_with(~ gsub("^swath_", "", .), starts_with("swath_")) %>%
  #drop years that were not surveyed
  filter(rowSums(is.na(.[, 10:ncol(.)])) != (ncol(.)-9)) %>%
  #drop any columns that are all NA
  dplyr::select(-which(sapply(., function(x) all(is.na(x))))) %>%
  #drop species that were never observed 
  dplyr::select(-which(sapply(., function(x) all(x == 0)))) %>%
  #make long
  pivot_longer(cols = c(10:ncol(.)), names_to="classcode", values_to="counts") %>%
  #drop substrate categories
  filter(!grepl("sub$",classcode))%>%
  #drop urchin tests and recruits
  filter(!grepl("rec$",classcode)) %>%
  filter(!grepl("test$",classcode))

#join with species names
swath_dat_build2 <- left_join(swath_dat_build1, kelp_taxon, by="classcode") %>%
  mutate(species_definition = tolower(str_replace_all(species_definition, " ", "_"))) %>%
  mutate(species_definition = ifelse(classcode == "macstipes", "macrocystis_pyriferastipes",species_definition)) %>%
  dplyr::select(!(classcode))%>%
  #check for any NAs in Viewer then reshape back to wide
  pivot_wider(names_from = "species_definition", values_from = "counts") %>%
  #drop species
...
upc_dat_build1 <- site_data %>%
  dplyr::select(1:9, starts_with("upc_cover")) %>%
  #drop string
  rename_with(~ gsub("^upc_cover", "", .), starts_with("upc_cover")) %>%
  #drop any columns that are all NA
  dplyr::select(-which(sapply(., function(x) all(is.na(x))))) %>%
  #drop species that were never observed 
  dplyr::select(-which(sapply(., function(x) all(x == 0)))) %>%
